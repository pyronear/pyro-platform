...
response_devices = requests.get(f"{cfg.API_URL}/devices/", headers=api_client.headers, timeout=5)
...
# Cache configuration
cache = Cache(app.server, config={"CACHE_TYPE": "filesystem", "CACHE_DIR": ".cache", "CACHE_DEFAULT_TIMEOUT": 60})
...
@app.callback(
    [Output("layer_style_button", "children"), Output("tile_layer", "url"), Output("tile_layer", "attribution")],
...
def zoom_on_alert(n_clicks, live_alerts, frame_urls, user_headers, user_credentials):
    ...
    if not ctx.triggered or all(elt is None for elt in n_clicks) or user_headers is None:
        ...
        lat, lon, div = build_alert_overview(
            live_alerts=live_alerts, frame_urls=frame_urls, event_id=event_id, acknowledged=acknowledged
        )
...
@app.callback(
    [Output("map", "center"), Output("map", "zoom")],
    [Input("login_zoom_and_center", "data"), Input("alert_zoom_and_center", "data")],
...
def close_alert_overview_main(
    alert_overview_style_zoom, alert_overview_style_closing_buttons, alert_overview_style_acknowledgement
):
    """
...
@app.callback(
    ...
    [
        ...
        Input({"type": "manage_confirmation_modal_close_button", "index": MATCH}, "children"),
        Input({"type": "manage_confirmation_modal_confirmation_button", "index": MATCH}, "children"),
    ],
)
...
@app.callback(
    Output({"type": "manage_confirmation_modal_close_button", "index": MATCH}, "children"),
...
@app.callback(Output("map_style_btn_switch_view", "children"), Input("map_style_button", "n_clicks"))
...
@app.callback(
    ...
    [Input("map_style_btn_switch_view", "children"), Input("alert_btn_switch_view", "children")],
    [State("map_style_button", "children"), State("current_map_style", "children")],
)
...
def update_live_alerts_components(live_alerts, map_style_button_label, images_url_live_alerts, site_devices_data):
    """
    ...
    if map_style == "alerts":
        ...
        output += build_individual_alert_components(live_alerts, images_url_live_alerts, site_devices_data)
...
@app.callback(Output("individual_alert_frame_placeholder", "children"), Input("images_url_live_alerts", "data"))
...
@app.callback(
    [Output("alert_frame_update_new_event", "children"), Output("last_displayed_event_id", "data")],
...
@app.callback(
    [Output("alert_frame", "src"), Output("interval-component-img-refresh", "n_intervals")],
    [Input("alert_frame_update_new_event", "children"), Input("alert_frame_update_interval", "children")],
)
