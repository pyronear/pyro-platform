...
def build_layer_style_button():
    ...
    """
    button = html.Div(dbc.Button(children="Satellite", id="layer_style_button", className="btn-layers"))
...
def build_filters_object(map_type):
    ...
    return html.Div(
        ...
        id=object_id,
        style={"position": "absolute", "bottom": "30px", "left": "10px", "z-index": "1000"},
    )
...
def build_legend_box(map_type=None):
    ...
    if map_type == "alerts":
        ...
        legend_body = [
            html.Div(
                [
                    html.Div(html.Img(src=site_img_url, style=img_style), style=image_div_style),
                    html.Div(html.P("Matériel installé"), style=text_div_style),
                ]
            )
        ]
        ...
        legend_body.append(
            html.Div(
                [
                    html.Div(html.Img(src=past_fire_img_url, style=img_style), style=image_div_style),
                    html.Div(html.P("Incendie passé"), style=text_div_style),
                ]
            )
        )
        ...
        legend_body.append(
            html.Div(
                [
                    html.Div(html.Img(src=alert_img_url, style=img_style), style=image_div_style),
                    html.Div(html.P("Alerte en cours"), style=text_div_style),
                ]
            )
        )
...
def build_historic_markers(dpt_code=None):
    ...
    for i, row in old_fire_positions.iterrows():
        ...
        date = datetime.strptime(row["acq_date"], "%Y-%m-%d").strftime("%d %b %Y")
        ...
        fire_markers.append(
            dl.Marker(
                id=f"historic_fire_{i}",  # Set an id for each marker to receive callbacks
                position=(lat, lon),
                icon=icon,
                children=[
                    dl.Tooltip(f"Date: {date}"),
                    dl.Popup(
                        [html.H4(f"Incendie du {date}"), html.P(f"Commune : {location}"), html.P(f"Type : {daynight}")]
                    ),
                ],
            )
        )
